        ...
        <b-row class="mt-4">
            <b-col cols="12">
                <b-table 
                    ...
                    responsive 
                    small
                    :items="for_approval_list"
                    :fields="for_approval_fields"
                    :per-page="perPage"
                    ...
                    <template #cell(action)="data">
                        <b-link 
                            ...
                            class="link_style"
                            @click="updateForApproval(data.index)">
                            Edit
                        </b-link>
                        <label class="ml-1 mr-1 text-secondary">|</label>
                        <b-link 
                            class="link_style"
                            @click="forApprovalSubparts()">
                            Subparts
                        </b-link>
                    </template>
                </b-table>
...
export default {
    ...
    computed:{
        rows() {
            return this.for_approval_list.length
        }
    },
    methods:{
        updateForApproval(id){
            this.status_name = "For Approval";
            this.status_id = "4";
            this.id = {};
            this.id = this.for_approval_list[id];
        },
        forApprovalSubparts(){
            alert("Open For Approval Subparts Modal");
        }
    }
}
</script>
